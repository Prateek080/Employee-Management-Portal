...
$x=$row['Under'];
 $y=$row['Id'];
  	...
  	echo	"</ul>";
	echo	"<h3>Project Manager</h3>";
	echo	"<ul class='toggle'>";
	echo		"<li class='icn_add_user'><a>". $x . "</a></li>";
	echo	"</ul>";
	
	echo	"<h3>Other Team Leaders</h3>";
	echo	"<ul class='toggle'>";
$result1= "SELECT Fname,Lname,Email FROM data WHERE Usertype='Teaml' and Email != '$user' and Under='$x'";
$run1 = mysql_query($result1) or die("ERROR".mysql_error());
while($row2=mysql_fetch_array($run1))
	{
$m=$row2['Email'];?>
	<li class='icn_add_user'><a href="teamembers.php?email=<?php echo $m; ?>"><?php echo $row2['Fname'] . "&nbsp;" . $row2['Lname'] ?></a></li>
		<?php
		}
	echo	"</ul>";
	
	echo	"<h3>Team Members under</h3>";
	echo	"<ul class='toggle'>";
$result1= "SELECT Fname,Lname FROM data WHERE Usertype='TeamM' and Under='$user' ";
$run1 = mysql_query($result1) or die("ERROR".mysql_error());
while($row2=mysql_fetch_array($run1))
	{
